...
#define OLED_PAGES 4      /* 32 / 8 = 4 */
#define OLED_COL_OFFSET 0 /* 128x32 屏一般从 0 开始 */
#define FB_SIZE (OLED_WIDTH * OLED_PAGES)
#define MAX_COLS (OLED_WIDTH / CHAR_W) /* 21 chars per line (128 / 6) */
...
static esp_err_t oled_init_display(void)
{
  ...
  //     0xA8,
  //     0x27, /* Multiplex ratio = 39 (40 rows) */
  //     0xD3,
...
static esp_err_t oled_flush(void)
{
  ...
  if (err != ESP_OK)
    return err;
  err = oled_cmd(OLED_COL_OFFSET);
  if (err != ESP_OK)
    return err;
  err = oled_cmd(OLED_COL_OFFSET + OLED_WIDTH - 1);
  if (err != ESP_OK)
    return err;
...
static void render_status(void)
{
  ...
  if (ap_ssid != NULL)
  {
    snprintf(line, sizeof(line), "WIFI SSID: %s", ap_ssid);
    fb_draw_string(0, line);
  }
  else
  {
    fb_draw_string(0, "NO AP");
  }
  ...
  if (ap_connect && my_ip != 0)
  {
    ip4_addr_t addr;
    addr.addr = my_ip;
    snprintf(line, sizeof(line), "IP: %d.%d.%d.%d", IP2STR(&addr));
    int len = strlen(line);
    fb_draw_string(1, len > MAX_COLS ? line + (len - MAX_COLS) : line);
  }
  else
  {
    fb_draw_string(1, "No IP");
  }
  ...
  if (ap_connect)
  {
    ...
    {
      snprintf(line, sizeof(line), "RSSI: %s %ddBm", status, ap_info.rssi);
    }
    else
    {
      snprintf(line, sizeof(line), "RSSI: %s", status);
    }
    fb_draw_string(2, line);
  }
  else
  {
    fb_draw_string(2, "DOWN");
  }
  ...
  // snprintf(line, sizeof(line), "Traffic: %.1f/%.1f MB",
