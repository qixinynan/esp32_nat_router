...
void app_main(void)
{
  ...
  ESP_LOGI(TAG, "Starting Ethernet-to-WiFi NAT Router...");
  ...
  {
    if (channel_setting >= 1 && channel_setting <= 13)
      ap_channel = (uint8_t)channel_setting;
  }
  if (ap_channel > 0)
  {
  ...
  if (vpn_private_key == NULL)
    vpn_private_key = param_set_default("");
  get_config_param_str("vpn_pubkey", &vpn_public_key);
  if (vpn_public_key == NULL)
    vpn_public_key = param_set_default("");
  get_config_param_str("vpn_psk", &vpn_preshared_key);
  if (vpn_preshared_key == NULL)
    vpn_preshared_key = param_set_default("");
  get_config_param_str("vpn_endpoint", &vpn_endpoint);
  if (vpn_endpoint == NULL)
    vpn_endpoint = param_set_default("");
  ...
  if (vpn_address == NULL)
    vpn_address = param_set_default("");
  get_config_param_str("vpn_mask", &vpn_netmask);
  if (vpn_netmask == NULL)
    vpn_netmask = param_set_default("255.255.255.0");
